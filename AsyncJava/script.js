...
//To show index number of even and odd numbers
function printEvenOddWithIndexAsync() {
    const numbers = ["Sadaf", "Ali", "Hira", "Amna"];
    document.getElementById("asyncOutput6").textContent = numbers.join(", ");
    const evenIndexes = [];
    const oddIndexes = [];
    let i = 0;
    document.getElementById('evenIndexOutputAsync').textContent = '';
    document.getElementById('oddIndexOutputAsync').textContent = '';
    function processNext() {
        if (i < numbers.length) {
            setTimeout(function() {
                if (i % 2 === 0) {
                    evenNumbers.push(numbers[i]);
                    document.getElementById('evenIndexOutputAsync').textContent = evenNumbers.join(', ');
                } else {
                    oddNumbers.push(numbers[i]);
                    document.getElementById('oddIndexOutputAsync').textContent = oddNumbers.join(', ');
                }
                i++;
                processNext();
            }, 200);
